...
- Affichage de la liste des tests pouvant être exécutés. La liste est filtrée sur le contexte que l'on donne dans la commande et on peut en plus filtrer sur ces tests dans l'interface.
...
- Localiser un élément dans le snapshot du dom (bouton Pick Locator).
...
Poser un point d'arrêt : `await page.pause();` (l'exécution s'arrête à cette ligne précise et ouvre automatiquement l'inspecteur, que l'on soit en mode --debug ou même hors debug).
...
- `npx playwright test tests/1-guest/cart.spec.ts` : lancer uniquement les tests d'un fichier précis.
- `npx playwright test --project=customer-tests` : lancer un "projet" de tests (ils sont déclarés dans le playwright.config.ts)
- `npx playwright test tests/1-guest/cart.spec.ts:42` : lancer uniquement le test situé à une ligne précise dans un fichier (pratique pour relancer un seul test rapidement).
- `npx playwright test tests/1-guest/` : lancer tous les tests d'un dossier.
...
- `npx playwright show-trace test-results/.../trace.zip` : ouvrir le Trace Viewer sur un fichier de trace généré lors d'un échec en CI.
